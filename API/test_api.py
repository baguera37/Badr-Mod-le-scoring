...
class APITestCase(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        # Lancer le serveur Flask dans un processus séparé
        cls.server_process = subprocess.Popen(["python", "api.py"])
        # Attendre quelques secondes pour que le serveur démarre
        time.sleep(2)

    @classmethod
    def tearDownClass(cls):
        # Arrêter le processus du serveur Flask
    ...
    def test_download_summary_plot_endpoint(self):
        ...
        response = requests.get('http://127.0.0.1:5000/download/summary_plot')
